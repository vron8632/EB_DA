...
class DACS(UDADecorator):
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img,
        ...
        if self.local_iter == 0:
            ...
            if not os.path.exists('mean_entropy.txt'):
                os.system('touch mean_entropy.txt')
            else:
                with open('mean_entropy.txt','a+') as f:
                    f.truncate(0)
            if os.path.exists('m_entropy.txt'):
                with open('m_entropy.txt','a+') as f1:
                    f1.truncate(0)
            else:
                os.system('touch m_entropy.txt')
        ...
        pseudo_weight_log_vars = {'pseudo_weight':pseudo_weight}
        log_vars.update(add_prefix(pseudo_weight_log_vars, 'l'))
        ...
        # 求学生模型在目标域和源域混合图像上推理结果的熵
        tgt_s_logits = self.get_model().encode_decode(mixed_img, img_metas)    # (2,19,512,512)
        tgt_s_softmax = torch.softmax(tgt_s_logits, dim=1)           # (2,19,512,512)
        matrix = tgt_s_softmax.permute(0, 2, 3, 1)                   # (2,512,512,19)
        ...
        ba_entropy = 0.4
        entropy_loss = 0.1 * torch.mean((entropy - ba_entropy) * (entropy - ba_entropy))
        entropy_log_vars = {'entropy': entropy_loss.item()}
        log_vars.update(add_prefix(entropy_log_vars, 'mix'))
